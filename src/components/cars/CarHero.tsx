...
export function CarHero({ vehicle, minPrice, minCarPrice, maxCarPrice }: CarHeroProps) {
  ...
  return (
    <section className="py-8 px-5 text-center bg-surface-secondary">
      <div className="relative w-full max-w-[280px] aspect-[5/3] mx-auto mb-4 rounded-2xl overflow-hidden bg-white border border-border-solid">
        {imageError ? (
          <div className="absolute inset-0 flex items-center justify-center text-text-sub text-sm">
            차량 이미지
          </div>
        ) : (
          <Image
      ...
      <h1 className="text-2xl font-extrabold text-text tracking-tight">{vehicle.model}</h1>
      <div className="flex gap-1.5 justify-center mt-2">
        <span className="px-2.5 py-1 rounded-[10px] text-[11px] font-semibold bg-primary/10 text-primary">
          {vehicle.segment}
        </span>
      ...
      <p className="text-[22px] font-extrabold text-primary mt-3">{priceText}</p>
    </section>
  );
}
