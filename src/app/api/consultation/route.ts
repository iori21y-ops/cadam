...
export async function POST(request: NextRequest) {
  try {
    ...
    // 텔레그램 알림
    try {
      const tgResult = await sendTelegramNotification(emailData);
      if (!tgResult.success) console.error("[Consultation] Telegram failed:", tgResult.error);
      await supabase.from("notification_log").insert({ consultation_id: consultation.id, channel: "telegram", status: tgResult.success ? "sent" : "failed", error_message: tgResult.error ?? null });
    } catch (tgErr) {
      console.error("[Consultation] Telegram send error:", tgErr);
    }
    // 11단계 - 응답
    return NextResponse.json({
      success: true,
