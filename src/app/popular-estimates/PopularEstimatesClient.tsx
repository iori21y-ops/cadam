...
export const PopularEstimatesClient = memo(function PopularEstimatesClient({
  ...
  return (
    <div className="flex flex-col gap-2">
      {vehicles.map((v, i) => {
        ...
        return (
          <Link
            key={v.id}
            href={`/cars/${v.slug}`}
            className="flex items-center gap-3 bg-white rounded-2xl border border-[#E5E5EA] p-3 hover:border-[#007AFF] hover:shadow-sm transition-all group"
          >
            <div
            ...
            <div className="w-20 h-12 relative shrink-0 bg-[#F5F5F7] rounded-xl overflow-hidden">
              <Image
            ...
            <div className="flex-1 min-w-0">
              <p className="font-bold text-sm text-[#1D1D1F] group-hover:text-[#007AFF] transition-colors truncate">
                {v.model}
              </p>
              ...
              {v.price ? (
                <p className="text-[#007AFF] font-semibold text-sm mt-0.5">
                  월 {formatPrice(v.price.min)}
                </p>
              ) : (
                <p className="text-[#86868B] text-xs mt-0.5">가격 준비중</p>
              )}
            </div>
      ...
      {vehicles.length === 0 && (
        <div className="py-16 text-center text-[#86868B]">등록된 차량이 없습니다</div>
      )}
    </div>
  );
});