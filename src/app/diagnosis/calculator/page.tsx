  ...
  return (
    <div className="min-h-screen bg-white pb-16">
      ...
      <div className="px-5 pt-10 max-w-lg mx-auto">
        {/* 헤더 */}
        <div className="mb-6">
          <p className="text-lg text-text-sub mb-1">월 비용 계산기</p>
        ...
        <div className="p-5 rounded-2xl bg-white border border-border-solid mb-4">
          <p className="text-lg font-bold text-text mb-3">차종 선택</p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <span className={`text-[15px] font-semibold truncate ${isSel ? 'text-white' : 'text-text'}`}>{v.name}</span>
                  </div>
                </SelectCard>
              );
            })}
        ...
        <div className="p-5 rounded-2xl bg-white border border-border-solid mb-4">
          <div className="mb-5">
            <div className="flex justify-between mb-2">
              <p className="text-lg font-bold text-text">계약 기간</p>
              <p className="text-lg font-bold text-primary">{months}개월</p>
            </div>
            <input
            ...
            />
            <div className="flex justify-between text-base text-text-muted mt-1">
              <span>24개월</span><span>72개월</span>
            </div>
          </div>
          <div>
            <div className="flex justify-between mb-2">
              <p className="text-lg font-bold text-text">선수금 비율</p>
              <p className="text-lg font-bold text-primary">{downPct}%</p>
            </div>
            <input
        ...
        {/* 결과 카드 */}
        <div className="p-5 rounded-2xl bg-white border border-border-solid mb-4">
          <p className="text-lg font-bold text-text mb-1">{selectedCar.brand} {selectedCar.name}</p>
          <p className="text-[15px] text-text-sub mb-5">{(selectedCar.price / 100).toFixed(1)}천만 · {months}개월 · 선수금 {downPct}%</p>
            ...
            {PRODUCT_KEYS.map((key) => {
              ...
              return (
                <div key={key} className={`flex items-center gap-3.5 rounded-2xl px-[18px] py-4 ${resultBgByKey[key]}`}>
                  <span className="text-[24px] shrink-0">{DEFAULT_PRODUCTS[key].emoji}</span>
                  <div className="flex-1">
                    <p className="text-lg font-semibold text-text">{DEFAULT_PRODUCTS[key].name}</p>
                    <p className="text-[13px] text-text-muted">{key === 'installment' ? `이자 ${(RATES.installment * 100).toFixed(1)}%` : key === 'lease' ? `리스 ${(RATES.lease * 100).toFixed(1)}%` : key === 'rent' ? '보험·정비 포함' : '일시불'}</p>
                  </div>
                    ...
                    {key === 'cash' ? (
                      ...
                      <>
                        <p className={`text-xl font-bold ${resultTextByKey[key]}`}>
                          {monthly.toLocaleString()}<span className="text-[15px]">만/월</span>
                        </p>
          ...
          <p className="text-[12px] text-text-muted mt-3">* 할부 {(RATES.installment * 100).toFixed(1)}%, 리스 {(RATES.lease * 100).toFixed(1)}%, 렌트 {(RATES.rent * 100).toFixed(1)}%+관리비 {Math.round(RENT_INSURANCE_RATE * 100)}% 기준 / 실제 조건에 따라 다를 수 있습니다.</p>
        </div>

        {/* 상담 CTA */}
        <div className="p-5 rounded-2xl bg-accent/10 mb-4">
          <p className="text-lg font-bold text-text mb-1">더 정확한 견적이 필요하신가요?</p>
          <p className="text-base text-text-sub mb-3">전문 상담사가 실제 조건으로 최저가를 찾아드립니다.</p>
