...
export default async function DiagnosisModelPage({
  ...
  return (
    <div className="h-dvh overflow-y-auto scroll-smooth snap-y snap-proximity bg-[#F2F2F7]">
      {/* 헤더 */}
      <div className="bg-white border-b border-[#E5E7EB] sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-5 py-4 flex items-center gap-3">
          <Link href="/diagnosis/report" className="text-[#007AFF] text-[15px]">
            ← 직접 진단
          </Link>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[13px] text-[#8E8E93]">5년 총비용 & 장기렌트 비교</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        {/* 차량 요약 배지 */}
        <div className="bg-white rounded-2xl shadow-sm border border-[#F2F2F7] p-5 snap-start scroll-mt-4">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1">
              <p className="text-[13px] text-[#8E8E93] mb-0.5">{vehicle.segment} · {vehicle.fuel}</p>
              <p className="text-[22px] font-bold text-[#1C1C1E]">
                {vehicle.brand} {vehicle.model}
              </p>
            </div>
            <span className="text-[13px] font-semibold px-2.5 py-1 rounded-full bg-[#007AFF18] text-[#007AFF]">
              {latestYear}년식
            </span>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={item.label} className="bg-[#F2F2F7] rounded-xl px-3 py-2.5">
                  <p className="text-[12px] text-[#8E8E93] mb-0.5">{item.label}</p>
                  <p className="text-[16px] font-bold text-[#1C1C1E]">{item.value}</p>
                  {item.sub && <p className="text-[12px] text-[#8E8E93]">{item.sub}</p>}
                </div>
              ))}
            </div>
          ) : (
            <p className="text-[15px] text-[#8E8E93] text-center py-4">가격 데이터를 불러오는 중입니다.</p>
          )}
        </div>
        ...
        {hasTrimData && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#F2F2F7] p-5 snap-start scroll-mt-4">
            <p className="text-[15px] font-bold text-[#1C1C1E] mb-3">
            ...
            <div className="space-y-2">
              {latestTrims.map((t) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <p className="text-[15px] font-medium text-[#1C1C1E]">{t.trim_name}</p>
                      <p className="text-[12px] text-[#8E8E93]">{fuelLabel(t.fuel_type)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[15px] font-bold text-[#1C1C1E]">{fmtMk(t.msrp_price)}</p>
                      <p className="text-[12px] text-[#8E8E93]">렌트 ≈ {fmtMk(rentMo)}/월</p>
                    </div>
                  </div>
                );
              })}
        ...
        {hasTrimData && midTrim && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#F2F2F7] p-5 snap-start scroll-mt-4">
            <p className="text-[15px] font-bold text-[#1C1C1E] mb-1">
              5년 장기렌트 예상 총비용
            </p>
            ...
            <div className="flex items-end justify-between px-4 py-3.5 bg-[#007AFF0D] rounded-xl">
              <div>
                <p className="text-[13px] text-[#8E8E93]">월 납입금</p>
                <p className="text-[24px] font-bold text-[#007AFF]">{fmtMk(monthlyRent)}</p>
              </div>
              <div className="text-right">
                <p className="text-[13px] text-[#8E8E93]">5년 납입 합계</p>
                <p className="text-[18px] font-bold text-[#1C1C1E]">{fmtMk(tco5yrRent)}</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#F2F2F7] p-5 text-center">
          ...
          <Link
            ...
            className="block w-full py-3.5 bg-[#007AFF] text-white text-[16px] font-bold rounded-2xl text-center hover:bg-[#0066CC] transition-colors"
          >
            무료 진단 시작하기
          </Link>
        </div>
        ...
        {/* 관련 차량 링크 */}
        <div className="bg-white rounded-2xl shadow-sm border border-[#F2F2F7] p-5 snap-start scroll-mt-4">
          <p className="text-[14px] font-bold text-[#8E8E93] uppercase tracking-wide mb-3">
            같은 브랜드 다른 모델
          </p>
            ...
            {VEHICLE_LIST
              ...
              .map((v) => (
                <Link
                  ...
                  className="text-[14px] text-[#007AFF] font-medium py-2 px-3 bg-[#F2F2F7] rounded-xl hover:bg-[#E5E7EB] transition-colors truncate"
                >
                  {v.model}
                </Link>
              ))}
        ...
        <p className="text-[12px] text-[#8E8E93] text-center px-2 pb-4">
